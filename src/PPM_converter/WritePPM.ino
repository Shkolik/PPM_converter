void setupWriter()                      //setup timer2
{
  ...
  OCR2 = 100;                           // count 100 times before transmit
  ...
  TCCR2B = bit (CS21);                  // 8 prescaler: 0,5 microseconds at 16mhz
  OCR2A = 100;                          // count 100 times before transmit
...
#ifdef __AVR_ATmega16__
ISR(TIMER2_COMP_vect)                   //compare interrupt routine
#else
...
{  
  ...
  if (state)                            // start pulse
  {
    ...
    { 
      if (onState)
        OUT_PORT |= outPinBitMask;      // change signal state
      else
  ...
  else                                  // state changed. wait for next channel pulse
  {
    if (channel >= channelAmount)       // all channels sended. calculate syncro gap
    {
      if (pulseRest == 0)
      {        
        if (!onState)                   // set signal to OFF state
          OUT_PORT |= outPinBitMask;
        else
          OUT_PORT &= ~outPinBitMask;
